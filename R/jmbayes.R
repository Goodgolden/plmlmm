function(lmeObject, newdata,
  ...
  ## match.arg matches a character arg against
  ## a table of candidate values as specified by choices.
  interval <- match.arg(interval)

  if (inherits(lmeObject, "lme")) {
    ## extraction from lme object
    data <- lmeObject$data
    ...
    times_orig <- times_orig[!is.na(times_orig)]
      ...
      ## extraction from the lmeComponents object
  ...
  ## no missing why it is important??
  newdata_nomiss <- newdata[complete.cases(newdata[all_vars]), ]
  ## because bayesian cannot predict the outcomes
  ## it has to come with the original dataset
  ...
  ## get the id done only works for one individual
  if (length(idVar) > 1) {
  ...
  ## is it in long format? or wide format?
  id_nomiss <- match(newdata_nomiss[[idVar]],
                     unique(newdata_nomiss[[idVar]]))

  ## creat empty vectors and matrices
  ...
  ## what is this??? the newdata, with id and time variables
  ...
  ## predicted y ??? based on the betas and Zbs
  ...
  ## generate M random variables with mean and variance
  ## for the fixed effects shown below
  ...
  ## The replacement forms replace values corresponding to
  ...
  for (m in seq_len(M)) {
    ...
    sampled_y[, m] <- if (interval == "confidence") {
      ...
      rnorm(n_pred, mean_m, lmeObject$sigma)
    }}
  ...
  if (!return_data) {
    ...
    list(times_to_pred = times_to_pred,
