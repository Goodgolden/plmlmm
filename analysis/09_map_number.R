...
## for the marginal model and simulation setups------------------------------
# all <- here::here("data",
#                   "epic",
...
lmm_pred <- function(train_data,
  ...
  lmm_results <-
    dplyr::select(lmmpred_50,
                  ...
                  centile25 = low,
                  centile75 = upp) %>%
    full_join(dplyr::select(lmmpred_80,
                            id, time,
                            # observed = ht,
                            pred,
                            centile10 = low,
...
seed <- runif(1, 0, 1) * 1000000
seed <- floor(seed)
simulation <- gen_all_ranef(seed = seed) %>% save_dataset()
...
print("this is bks_pred_knots for testing --------------------")
print(head(bks_pred_knots))
...
print("3. test and train pred works ------------------------")
print(head(test_pred))

## 1.2 linear fitting

# bks_pred = test_pred
lm_bks_train <- lm("`.pred` ~ timef * sex + baseline",
                   data = train_pred)
predicted_train <- predict(lm_bks_train, newdata = train_pred)
lb_train <- train_pred %>%
  ungroup() %>%
...
print("this is lb_train!!!!!")
head(lb_train)

print("4. linear brokenstick works--------------------------")

lm_bks_test <- lm("`.pred` ~ timef * sex + baseline",
                  data = test_pred)
predicted_test <- predict(lm_bks_test, newdata = test_pred)
test_pred[, "lm_bks_target"] = as.numeric(predicted_test)
lb_test <- test_pred %>%
  dplyr::select(contains(c("id", "time", "lm_bks")))

print("this is lb_test!!!!")
head(lb_test)

test_eld_all <- map(c(3, 5, 8, 10, 15, 20, 25, 30, 50),
                    function(number){
                      map(id_test,
                          ~pred_matching(
                            lb_data = lb_train,
...
save(test_eld_all, file = "test_eld_20221020.Rdata")
remove(test_eld_all)


### test_mhl_n10 ----------------
test_mhl_nall <- map(c(3, 5, 8, 10, 15, 20, 25, 30, 50),
                    function(number){
                      map(id_test,
                          ~pred_matching(
                            lb_data = lb_train,
                            ...
                            predict_plot = FALSE,
                            sbj = .))})
save(test_mhl_nall, file = "test_mhl_n_20221020.Rdata")
remove(test_mhl_nall)

### test_mhl_p09-------------------
test_mhl_pall <- map(c(0.8, 0.85, 0.88, 0.90, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98),
                    function(number) {
                      map(id_test,
                          ~try(pred_matching(
                            lb_data = lb_train,
                            ...
                            predict_plot = FALSE,
                            sbj = .)))})
save(test_mhl_pall, file = "test_mhl_p_20221020.Rdata")
remove(test_mhl_pall)


## test_sgl10_n10-----------------------
test_sgl_tall <- map(c(2,4, 6, 8, 10, 12, 14),
                      function(number) {
                        map(id_test,
                            ~pred_matching(
                              lb_data = lb_train,
...
save(test_sgl_tall, file = "test_sgl_t_20221020.Rdata")
